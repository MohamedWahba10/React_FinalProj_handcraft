...
import styles from "./UpdateProductAdmin.module.css";
...
export default function UpdateProductAdmin() {
  ...
  async function callUpdateProduct(values) {
    setisLoading(true);
    const prodOnSaleValue = values.prodOnSale ? "True" : "False";
    ...
    formData.append("prodVendor", values.prodVendor);
    formData.append("prodStock", values.prodStock);
    formData.append("prodOnSale", prodOnSaleValue);
    formData.append(
    ...
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/api/panel/updateproduct/${id}/`,
        formData,
      ...
      setError(
        error.response.data.prodImageFour ||
          error.response.data.prodImageOne ||
          error.response.data.prodImageTwo ||
          error.response.data.prodImageThumbnail ||
          error.response.data.prodImageThree
      );
  ...
  const validationSchema = Yup.object({
    ...
    prodVendor: Yup.string().required("Name Vendor Is Required"),
  ...
  useEffect(() => {
    if (productData) {
      UpdateProductForm.setValues({
        ...
        prodVendor: productData.prodVendor?.id || "",
  ...
  async function AllUser() {
    try {
      let response = await axios.get(`http://127.0.0.1:8000/api/allUser/`, {
        headers: {
          Authorization: `Token ${localStorage.getItem("userToken")}`,
        },
      });
  ...
  return (
    <>
      <div className={`${styles.header_UpdateProduct} py-5 mb-5 mx-5 px-5 `}>
        <Link to="/adminPanel/adminProduct" className="text-decoration-none ">
          <span className={`${styles.link_home} pe-1 `}>Product</span>
        </Link>

        <span className={`${styles.span_AddProduct}`}>/ Update Product</span>

        <div className="container my-5">
          <div>
            <h2 className="text-center my-3">Update Product</h2>
              ...
              >
                <div className="row gy-4">
                  ...
                  <div className="col-md-12">
                    <div className="form-group">
                      <label htmlFor="prodVendor" className="fs-4 fw-bold">
                        Product Vendor
                      </label>
                      ...
                      <select
                        className="w-100 "
                        id="prodVendor"
                        value={UpdateProductForm.values.prodVendor}
                        name="prodVendor"
                        onChange={UpdateProductForm.handleChange}
                        onBlur={UpdateProductForm.handleBlur}
                      >
                        ...
                        {Array.isArray(userData) &&
                          userData
                            ...
                            .map((ven) => (
                              <option value={ven.id} key={ven.id} 
                              
                             
                              selected={
                                ven.id ===
                                UpdateProductForm.values.prodVendor
                              }
                      ...
                      {UpdateProductForm.errors.prodVendor &&
                        UpdateProductForm.touched.prodVendor && (
                          <div className="text-danger fs-5 mt-3">
                            {UpdateProductForm.errors.prodVendor}
                          </div>
                        )}
                    ...
                    <div className="col-md-12">
                      <div className="form-group">
                        ...
                        <input
                          ...
                          value={
                            UpdateProductForm.values.prodDiscountPercentage
                          }
