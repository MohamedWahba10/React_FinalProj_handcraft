...
export default function ProductINSubCategory() {
  const { subCategoryId,subCategoryName,categoryName,categoryId} = useParams();
  ...
  return (
    <>
      ...
      <div className={`${styles.header_profile} py-5 mb-5 text-center `}>
        ...
        <Link to={`/subcategory/${categoryId}/${categoryName}`} className="text-decoration-none ">
          ...
          <div className="container my-5">
            <div className="row gy-4">
              {product
                ?.filter(
                  (productItem) =>
                    productItem.prodSubCategory == subCategoryId
                )
                .map((productItem) => (
